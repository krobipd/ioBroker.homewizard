...
- **Version:** 0.9.0 (released 2026-05-23) — Preserve user-modified state names (`extendObjectAsync` with `{ preserve: { common: ["name"] } }`). State name translations migrated from private `i18n-states.ts` to adapter-core `I18n.getTranslatedObject()` / `I18n.translate()`. admin/i18n Pattern A→B (flat files, 119 keys). bonjour-service v1.4.0 TS 6 type compat fix. 231 unit + 57 package = 288 tests. Vorgänger **0.8.3** (released 2026-05-22) — Community-standard event handler pattern. Vorgänger **0.8.2** (released 2026-05-19) — NUT-Konsistenz. Vorgänger **0.8.0** (released 2026-05-17) — Toolchain-Parity: TypeScript ~6.0.3, vitest, eslint-config 2.3.4.
