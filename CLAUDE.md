...
- **Version:** 0.8.3 (released 2026-05-22) — Community-standard event handler pattern (`.bind(this)` + try/catch statt `.catch()` call-site). `npm update` aus `.releaseconfig.json` entfernt. Vorgänger **0.8.2** (released 2026-05-19) — NUT-Konsistenz: prettier ioBroker-Standard, dependabot double-quotes + TS-6-Kommentar, CI `fail_level: error`, `.releaseconfig.json` 2-Space, vitest `singleFork: false`, README Claude-footer-Fix. Vorgänger **0.8.0** (released 2026-05-17) — Toolchain-Parity: TypeScript ~6.0.3, vitest statt mocha+chai, eslint-config 2.3.4, release-script 5.2.0.
