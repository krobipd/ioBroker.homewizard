...
- **Version:** 0.12.1 (released 2026-06-09, npm latest — der deploy-Job 409te zunächst am transienten sigstore-`TLOG_CREATE_ENTRY_ERROR`; ein **zeitversetzter** deploy-Re-Run brachte 0.12.1 sauber durch [sofortige Re-Runs 409ten wg. deterministischem Build]; Memory `feedback_ci_flake_rerun`) — Internes Konsistenz-Patch nach ergebnis-offener Tiefen-Analyse (alle 23 Files vollständig gelesen; Verdikt: sauberster der drei Audit-Adapter — vier Vor-Audits [v0.10.0/v0.9.3/v0.7.5/v0.6.3] zeigen sich, **null** substanzielle Funde). **Toter Getter:** `HomeWizardWebSocket.isConnected` raus (kein Prod-Caller — main nutzt `conn.wsAuthenticated`; gleiche Kategorie wie hueemu `running`/hassemu `getCached`; 3 Test-Asserts auf internen `ws`-State umgestellt). **Tautologie-Test gelöscht:** `expect(UNSTABLE_DISCONNECT_THRESHOLD).toBe(3)` — interner Tuning-Wert-Spiegel, Verhalten bereits in main.test `isUnstable` + main-helpers.test `decideUnstableTransition` getestet (redundant, kein Coverage-Verlust). Kein Verhaltens-Change → **Patch**, Changelog „Internal refactoring. No user-facing changes." 259 unit + 57 package = 316. Alle Gates grün.
