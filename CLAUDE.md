...
- **Version:** 0.10.0 (released 2026-06-01, API-v2-Vollständigkeit: predictive-Batteriemodus + charge_to_full-Switch, WS-Echtzeit-Push für system + batteries (additiv, REST-Poll bleibt für uptime/rssi), wifi_ssid-State, rohes P1-telegram (P1-only), Token-Revoke beim Entfernen (DELETE /api/user). KISS/DRY (tDesc raus, onWs*-Methoden extrahiert, createState→Options-Objekt), M1 process-Handler raus, M2 setStateChangedAsync für langsame Felder. main.ts-Orchestrierungs-Tests + realer wss-Stub. Deps bonjour-service 1.4.0 + ws 8.21.0. W3042/W6028 gefixt; W5004/5005 durch Umbenennen der TimerDeps-DI-Methoden (schedule/cancel/...) gelöst — KEINE repochecker-Ausnahme). Vorgänger **0.9.3** (released 2026-05-24, memory/perf audit: TimerDeps-Injection, preserve device names). 264 unit + 57 package = 321 tests.
...
## Tests (264 unit + 57 package = 321)

```
src/main.test.ts                      → orchestration: onStateChange-routing (reboot/identify/setSystem/mode-validate/permissions/charge_to_full), removeDevice (deleteUser+teardown), isUnstable, onWs* push-handlers (connected-reset, battery_count-gate, removed-guard, unstable-backoff), pollSystemInfo telegram-branch (A5). Stub Adapter-base + injected makeClient/makeWebSocket (19)
