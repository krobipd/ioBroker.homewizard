...
- **Version:** 0.8.1 (released 2026-05-18) — Internal cleanup: dead tsconfig settings entfernt (noEmitOnError, outDir, removeComments, sourceMap, inlineSourceMap — alle no-ops bei noEmit:true). Vorgänger **0.8.0** (released 2026-05-17) — Toolchain-Parity: TypeScript ~6.0.3, vitest statt mocha+chai, eslint-config 2.3.4, release-script 5.2.0. `asName()` no-op entfernt. `sync-iopackage-from-i18n.py` (hassemu/beszel-Linie). extIcon raw→jsdelivr (CSP-Fix). nyc/source-map-support/ts-node raus. `pre-release.py --audit-current` Hook.
