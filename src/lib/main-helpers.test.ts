...
describe("decideUnstableTransition", () => {
  ...
  it("noChange when first short disconnect (counter goes 0 → 1)", () => {
    expect(decideUnstableTransition(0, 100, STABLE, THRESHOLD)).toBe("noChange");
  });

  it("noChange on second short disconnect (1 → 2, threshold not yet hit)", () => {
    expect(decideUnstableTransition(1, 100, STABLE, THRESHOLD)).toBe("noChange");
  });

  it("becameUnstable exactly when third short disconnect crosses the threshold", () => {
    expect(decideUnstableTransition(2, 100, STABLE, THRESHOLD)).toBe("becameUnstable");
  });

  it("noChange when long-lived connection but never was unstable", () => {
    expect(decideUnstableTransition(0, 700_000, STABLE, THRESHOLD)).toBe("noChange");
    expect(decideUnstableTransition(2, 700_000, STABLE, THRESHOLD)).toBe("noChange");
  });

  it("stabilized when long-lived connection AND was previously unstable", () => {
    expect(decideUnstableTransition(3, 700_000, STABLE, THRESHOLD)).toBe("stabilized");
    expect(decideUnstableTransition(5, 700_000, STABLE, THRESHOLD)).toBe("stabilized");
  });

  it("does not re-emit becameUnstable past the threshold (stays noChange)", () => {
    expect(decideUnstableTransition(3, 100, STABLE, THRESHOLD)).toBe("noChange");
    expect(decideUnstableTransition(10, 100, STABLE, THRESHOLD)).toBe("noChange");
  });
});
...
describe("computeReconnectDelay", () => {
  ...
  it("doubles each subsequent attempt", () => {
    expect(computeReconnectDelay(2, BASE, MAX)).toBe(10_000);
    expect(computeReconnectDelay(3, BASE, MAX)).toBe(20_000);
    expect(computeReconnectDelay(4, BASE, MAX)).toBe(40_000);
  });

  it("caps at max (stable)", () => {
    expect(computeReconnectDelay(20, BASE, MAX)).toBe(MAX);
  });

  it("caps at smaller max (unstable mode)", () => {
    expect(computeReconnectDelay(20, BASE, 60_000)).toBe(60_000);
  });
...
describe("shouldStartIpRecovery", () => {
  ...
  it("false until the threshold is reached", () => {
    expect(shouldStartIpRecovery(0, BEFORE, RETRY_EVERY)).toBe(false);
    expect(shouldStartIpRecovery(2, BEFORE, RETRY_EVERY)).toBe(false);
  });

  it("true on the exact threshold (first run)", () => {
    expect(shouldStartIpRecovery(3, BEFORE, RETRY_EVERY)).toBe(true);
  });

  it("false between retries", () => {
    expect(shouldStartIpRecovery(4, BEFORE, RETRY_EVERY)).toBe(false);
...
describe("stripNamespace", () => {
  it("strips a matching prefix", () => {
    expect(stripNamespace("homewizard.0.hwe-p1_aabb.measurement.power_w", "homewizard.0")).toBe(
      "hwe-p1_aabb.measurement.power_w",
    );
  });

  it("returns the input unchanged when prefix does not match", () => {
    expect(stripNamespace("other.adapter.foo", "homewizard.0")).toBe("other.adapter.foo");
  });

  it("does not strip when the namespace is a partial substring (must be followed by '.')", () => {
    expect(stripNamespace("homewizard.0extra.foo", "homewizard.0")).toBe("homewizard.0extra.foo");
  });
});
...
describe("findConnectionForState", () => {
  ...
  it("returns the matching connection by prefix", () => {
    ...
    expect(findConnectionForState("homewizard.0.hwe-bat_ccdd.battery.mode", "homewizard.0", conns)).toBe(b);
    expect(findConnectionForState("homewizard.0.hwe-p1_aabb.measurement.power_w", "homewizard.0", conns)).toBe(a);
  });

  it("returns undefined for an unknown prefix", () => {
    const a = fakeConnection("hwe-p1_aabb");
    const conns: [string, DeviceConnection][] = [["hwe-p1_aabb", a]];
    expect(findConnectionForState("homewizard.0.somewhereElse.foo", "homewizard.0", conns)).toBeUndefined();
  });

  it("returns undefined when state-ID is the prefix itself (no child)", () => {
    const a = fakeConnection("hwe-p1_aabb");
    const conns: [string, DeviceConnection][] = [["hwe-p1_aabb", a]];
    expect(findConnectionForState("homewizard.0.hwe-p1_aabb", "homewizard.0", conns)).toBeUndefined();
  });
  ...
  it("does not match a partial prefix (e.g. hwe-p1 vs hwe-p1_aabb)", () => {
    ...
    expect(findConnectionForState("homewizard.0.hwe-p1.foo", "homewizard.0", conns)).toBeUndefined();
  });
});
...
describe("shouldEmitAfterCooldown", () => {
  ...
  it("emits when never seen before (lastMs=0)", () => {
    expect(shouldEmitAfterCooldown(0, 1_700_000_000_000, COOLDOWN)).toBe(true);
  });

  it("suppresses when same instant as last emit", () => {
    expect(shouldEmitAfterCooldown(1_700_000_000_000, 1_700_000_000_000, COOLDOWN)).toBe(false);
  });

  it("suppresses well inside the window (30 min after)", () => {
    const last = 1_700_000_000_000;
    expect(shouldEmitAfterCooldown(last, last + 30 * 60 * 1000, COOLDOWN)).toBe(false);
  });

  it("emits at the window boundary (exactly cooldownMs later)", () => {
    const last = 1_700_000_000_000;
    expect(shouldEmitAfterCooldown(last, last + COOLDOWN, COOLDOWN)).toBe(true);
  });

  it("emits well past the window (1h + 1 min)", () => {
    const last = 1_700_000_000_000;
    expect(shouldEmitAfterCooldown(last, last + COOLDOWN + 60_000, COOLDOWN)).toBe(true);
  });
